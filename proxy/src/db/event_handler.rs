...
pub trait EventHandler {
	fn handle_events(&mut self, con: &tsclientlib::Connection, events: &[Event]) -> Result<(), Error>;
}
		...
		for e in events {
			match e {
				...
				Event::PropertyRemoved { id, old, .. } => {
					match id {
						PropertyId::Client(_) => {
							...
							diesel::update(servers_clients::table
								.filter(servers_clients::server.eq(server)
						...
						PropertyId::Channel(ch_id) => {
							...
							diesel::update(channels.filter(server.eq(&ch_server)
				...
				Event::Message { from, invoker, message } => {
					use schema::{channel_chats, client_chats, client_pokes, messages, server_chats};
					...
					match from {
						MessageTarget::Server => {
							...
							chat = self.con.transaction::<_, diesel::result::Error, _>(|| {
								if let Some(chat) = server_chats::table.find(server)
									...
									diesel::insert_into(server_chats::table)
										.values(&(server_chats::server.eq(server),
						...
						MessageTarget::Channel => {
							...
							chat = self.con.transaction::<_, diesel::result::Error, _>(|| {
								if let Some(chat) = channel_chats::table.find((server, channel))
									...
									diesel::insert_into(channel_chats::table)
										.values(&(channel_chats::server.eq(server),
											channel_chats::channel.eq(channel),
						...
						MessageTarget::Client(id) => {
							...
							chat = self.con.transaction::<_, diesel::result::Error, _>(|| {
								if let Some(chat) = client_chats::table.find((server, &client))
									...
									diesel::insert_into(client_chats::table)
										.values(&(client_chats::server.eq(server),
						...
						MessageTarget::Poke(id) => {
							...
							chat = self.con.transaction::<_, diesel::result::Error, _>(|| {
								if let Some(chat) = client_pokes::table.find((server, &client))
									...
									diesel::insert_into(client_pokes::table)
										.values(&(client_pokes::server.eq(server),
					...
					self.last_message_id = self.con.transaction::<_, diesel::result::Error, _>(|| {
						if can_be_duplicate {
							...
							let id = messages::table.filter(cmp)
								.select(messages::id)
								.first::<i64>(&self.con)
								.optional()?;
						...
						let message = models::MessageInsert {
							chat,
							invoker: invoker_uid.as_ref().map(|v| v.as_slice()),
							invoker_name,
							content: message,
							time: &utc_time,
							timezone: utc_to_local_offset,
						...
						diesel::insert_into(messages::table)
							.values(&message)
							.execute(&self.con)?;
						messages::table.order(messages::id.desc())
							.select(messages::id)
							.first::<i64>(&self.con)
					})?;
				}
