<script lang="ts">
	...
	import { draggable, DragData } from "../ui/util/draggable";
	import {
		findParent,
		flash,
		LOUDNESS_HISTORY,
	...
	<div bind:this={div} tabindex="0">
		...
		>
			...
			<span class="nameBox" style="color:{$client.color};">
				{#if showId}
					[<HighlightString filter={thisFilter} content={$client.id.toString()} />]
				{/if}
				<HighlightString filter={showId ? "" : thisFilter} content={$client.name} />
			</span>
			<span class="icons">
