...
import { getClientIconPath, getIconPath, IconSourceLike } from "./ui/clientIcon";
...
export type NotificationHandler = (
	...
	notification?: { title: TsNotification, content?: TsNotification, icon?: IconSourceLike, options?: NotificationOptions },
) => void;
	...
	try {
		...
		if ("Message" in msg) {
			...
			if (isPoke(msg.Message.target)) {
				...
				const isPrivateMsg = Object.prototype.hasOwnProperty.call(msg.Message.target, "Client");
				// If in quiet mode, ignore all but private messages
				if (!isQuiet || isPrivateMsg) {
					let tts;
					if (longMessage)
						tts = notif`${msg.Message.invoker} wrote a message`;
					else
						tts = notif`${msg.Message.invoker} wrote ${msg.Message.message}`;
			...
			if ("PropertyAdded" in msg) {
				...
				if ("Channel" in prop) {
					if (con.getState().channelListFinished) {
						...
						let tts;
						if (invoker !== null)
							tts = notif`${invoker} created ${channel}`;
						else
							tts = notif`Channel ${channel} created`;
						handler(con, msg, NotificationCategory.ChannelChanged, false, tts,
							{ title: notif`${con.book.server}`, content: tts, icon: channel });
					}
					...
					if (
						...
						if (client.id === ownClientId) {
							handler(con, msg, NotificationCategory.ClientSwitched, true,
								notif`Connected to ${con.book.server}`,
								{ title: notif`${con.book.server}`, content: notif`Connected`, icon: con.book.server });
						} else if (!isQuiet) {
							let tts;
							if (client.channel === ownChannelId)
								tts = notif`${client} connected`;
							else
								tts = notif`${client} connected to ${con.book.getChannel(client.channel)}`;

							handler(con, msg, NotificationCategory.ClientSwitched, client.channel === ownChannelId, tts,
								{ title: notif`${con.book.server}`, content: notif`${client} connected`, icon: client });
						}
						...
						if (!isQuiet || client.id === ownClientId) {
							if (client.channel === ownChannelId) {
								...
								if (invoker !== null)
									...
									tts = notif`${client} was moved in and appeared`;
					...
					if (!isQuiet) {
						...
						if (invoker !== null)
							...
							tts = notif`${client} was added to group ${group}`;
						handler(con, msg, NotificationCategory.ClientChanged, client?.id === ownClientId, tts);
					}
				}
				...
				if ("Channel" in prop && "Channel" in msg.PropertyChanged.id) {
					if (!isQuiet && msg.PropertyChanged.extra.reason === Reason.Channeledit) {
						...
						if (isInteresting) {
							...
							let tts;
							if (invoker !== null)
								tts = notif`${invoker} edited ${channel}`;
							else
								tts = notif`${channel} was edited`;
							handler(con, msg, NotificationCategory.ChannelChanged, channel.id === ownChannelId, tts);
						}
					}
					...
					if (newClient.channel !== undefined) {
						...
						if (reason === Reason.None) {
							if (client.id === ownClientId) {
								handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Switched to ${newChannel}`);
							} else if (!isQuiet) {
								if (newChannel.id === ownChannelId)
									handler(con, msg, NotificationCategory.ClientSwitched, true, notif`${client} joined`);
								else if (client.channel === ownChannelId)
									handler(con, msg, NotificationCategory.ClientSwitched, true, notif`${client} left to ${newChannel}`);
								else handler(con, msg, NotificationCategory.ClientSwitched, false, notif`${client} switched to ${newChannel}`);
							}
						} else if (reason === Reason.Moved) {
							if (client.id === ownClientId) {
								if (invoker !== null)
									handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Moved to ${newChannel} by ${invoker}`);
								else handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Moved to ${newChannel}`);
							} else if (!isQuiet) {
								if (newChannel.id === ownChannelId) {
									if (invoker !== null)
										handler(
											con,
											msg,
											NotificationCategory.ClientSwitched, true,
											notif`${client} was moved in by ${invoker}`,
										);
									else handler(con, msg, NotificationCategory.ClientSwitched, true, notif`${client} was moved in`);
								} else if (client.channel === ownChannelId) {
									if (invoker !== null)
										handler(
											con,
									...
									if (invoker !== null)
										handler(
											con,
											msg,
											NotificationCategory.ClientSwitched, false,
							...
							if (client.id === ownClientId) {
								if (invoker !== null)
									handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Kicked to ${newChannel} by ${invoker}`);
								else handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Kicked to ${newChannel}`);
							} else if (!isQuiet) {
								if (newChannel.id === ownChannelId) {
									if (invoker !== null)
										handler(
									...
									else handler(con, msg, NotificationCategory.ClientSwitched, true, notif`${client} was kicked in`);
								} else if (client.channel === ownChannelId) {
									if (invoker !== null)
										handler(
											con,
					...
					if (!isQuiet && newClient.name !== undefined) {
						handler(con, msg, NotificationCategory.ClientChanged, client.id === ownClientId, notif`${client} is now known as ${newClient.name}`);
					}

					if (!isQuiet && newClient.awayMessage !== undefined) {
						if (newClient.awayMessage === null)
							handler(con, msg, NotificationCategory.ClientStateChanged, client.id === ownClientId, notif`${client} is back`);
						else if (newClient.awayMessage!.length === 0)
							handler(con, msg, NotificationCategory.ClientStateChanged, client.id === ownClientId, notif`${client} has gone`);
						else
							handler(
								con,
								msg,
								NotificationCategory.ClientStateChanged, client.id === ownClientId,
					...
					if (newClient.inputMuted !== undefined) {
						if (client.id === ownClientId) {
							if (newClient.inputMuted) handler(con, msg, NotificationCategory.ClientStateChanged, true, notif`muted`);
							else handler(con, msg, NotificationCategory.ClientStateChanged, true, notif`unmuted`);
						} else if (!isQuiet && inOwnChannel) {
							if (newClient.inputMuted) handler(con, msg, NotificationCategory.ClientStateChanged, false, notif`${client} is muted`);
							else handler(con, msg, NotificationCategory.ClientStateChanged, false, notif`${client} is unmuted`);
						}
					}
					...
					if (
						...
						if (newClient.outputMuted) handler(con, msg, NotificationCategory.ClientStateChanged, client.id === ownClientId, notif`${client} is deaf`);
						else handler(con, msg, NotificationCategory.ClientStateChanged, client.id === ownClientId, notif`${client} is listening`);
					}
					...
					if (
						...
						if (newClient.inputHardwareEnabled)
							handler(con, msg, NotificationCategory.ClientStateChanged, client.id === ownClientId, notif`${client} can talk`);
						else handler(con, msg, NotificationCategory.ClientStateChanged, client.id === ownClientId, notif`${client} is silent`);
					}
				} else if ("Server" in prop) {
					if (!isQuiet && invoker !== null) {
						handler(con, msg, NotificationCategory.ChannelChanged, true, notif`${invoker} edited the server`);
					} else {
				...
				if ("Channel" in msg.PropertyRemoved.id) {
					...
					if (
						...
						if (client.id === ownClientId)
							handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Disconnected from ${con.book.server}`);
						else if (!isQuiet) handler(con, msg, NotificationCategory.ClientSwitched, client.channel === ownChannelId, notif`${client} disconnected`);
					} else if (reason === Reason.LostConnection) {
						if (client.id === ownClientId)
							handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Timed out from ${con.book.server}`);
						else if (!isQuiet) handler(con, msg, NotificationCategory.ClientSwitched, client.channel === ownChannelId, notif`${client} timed out`);
					} else if (reason === Reason.Moved) {
						if (client.channel === ownChannelId) {
							if (invoker !== null) {
								if (invoker.id === 0) {
									...
									handler(con, msg, NotificationCategory.ClientSwitched, true, notif`${client} was moved out by ${invoker}`);
								} else {
									handler(
										con,
										msg,
								...
								handler(con, msg, NotificationCategory.ClientSwitched, true, notif`${client} was moved out and disappeared`);
						} else if (!isQuiet) {
							if (invoker !== null) {
								if (invoker.id === 0) {
									// This could be a new channel we are not subscribed to yet
									// TODO Wait <ping> and check if we got the client again before talking
									handler(con, msg, NotificationCategory.ClientSwitched, false, notif`${client} was moved by ${invoker}`);
								} else {
									handler(
										con,
										msg,
							...
							} else handler(con, msg, NotificationCategory.ClientSwitched, false, notif`${client} was moved and disappeared`);
						}
					} else if (reason === Reason.KickChannel) {
						if (!isQuiet) {
							if (client.channel === ownChannelId) {
								...
								else handler(con, msg, NotificationCategory.ClientSwitched, false, notif`${client} was kicked and disappeared`);
							}
						}
					} else if (reason === Reason.KickServer) {
						if (client.id === ownClientId) {
							if (invoker !== null)
								handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Kicked from the server by ${invoker}`);
							else handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Kicked from the server`);
						} else if (!isQuiet) {
							if (invoker !== null)
								handler(
									con,
									msg,
									NotificationCategory.ClientSwitched, client.channel === ownChannelId,
									notif`${client} was kicked from the server by ${invoker}`
								);
							else handler(con, msg, NotificationCategory.ClientSwitched, client.channel === ownChannelId, notif`${client} was kicked from the server`);
						}
					} else if (reason === Reason.KickServerBan) {
						if (client.id === ownClientId) {
							if (invoker !== null)
								handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Banned from the server by ${invoker}`);
							else handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Banned from the server`);
						} else if (!isQuiet) {
							if (invoker !== null)
								handler(
									con,
							...
							else handler(con, msg, NotificationCategory.ClientSwitched, client.channel === ownChannelId, notif`${client} was banned from the server`);
						}
					} else if (
						...
						handler(con, msg, NotificationCategory.ClientSwitched, true, notif`Disconnected, server ${con.book.server} shut down`);
					}
					...
					if (!isQuiet) {
						...
						if (invoker !== null)
							handler(
								con,
								msg,
								NotificationCategory.ClientChanged, client?.id === ownClientId,
								notif`${invoker} removed ${client} from group ${group}`
							);
						else handler(con, msg, NotificationCategory.ClientChanged, client?.id === ownClientId, notif`${client} was removed from group ${group}`);
					}
				}
			}
		}
		...
		if ("ChannelDescriptionChanged" in msg) {
			if (!isQuiet) {
				for (const c of msg.ChannelDescriptionChanged) {
					...
					handler(con, msg, NotificationCategory.ClientChanged, ownClient?.channel === channel.id, notif`${channel}’s description was edited`);
					//}
				}
			}
		}
...
function defaultNotificationHandler(
	...
	if (!isRelevant && "onlyRelevant" in settings && settings.onlyRelevant)
		return;
	if (settings.tts) {
		...
		if (!handled)
			app.transientSettings.synth.trySpeak(tts.toString(con, true));
	}
	if (settings.notification && app.transientSettings.app.allowBrowserNotifications) {
		...
		const iconPath = notification?.icon ? getClientIconPath(notification.icon, con) : getIconPath(con.book.server, con);
		let options = {
			body: notification === undefined ? tts.toString(con, false) : notification.content?.toString(con, false),
			icon: iconPath,
			badge: "/icon.png",
		};
		if (notification?.options !== undefined)
			Object.assign(options, notification.options);
		new Notification(notification === undefined ? notif`${con.book.server}`.toString(con, false) : notification.title.toString(con, false), options);
	}
